...
def render(mode, stats):
    ...
    out = [
        ...
        f'<rect x="0.5" y="0.5" width="979" height="499" rx="10" fill="{p["bg"]}" stroke="{p["border"]}"/>',
    ]
    ...
    for i, segs in enumerate(info_lines(stats)):
        ...
        out.append(f'<text x="530" y="{45 + i * 21}" xml:space="preserve">{spans}</text>')
    out.append("</svg>")
    return "\n".join(out)
